...
class SubAccount(BaseCase):
    ...
    @parameterized.expand([add_account_success_random(7),
                           add_account_success_random(6),
                           add_account_success_random(5),
                           add_account_success_random(4),
                           add_account_success_random(12),
                           add_account_success_random(8),
                           add_account_success_random(9),
                           add_account_success_random(16),
                           add_account_success_random(13),
                           add_account_success_random(17),
                           add_account_success_random(18),])
    ...
    @parameterized.expand([add_account_false_ramdom(9),
                           add_account_false_ramdom(7),
                           add_account_false_ramdom(4),
                           add_account_false_ramdom(3),
                           add_account_false_ramdom(8),
                           add_account_false_ramdom(10),
                           add_account_false_ramdom(12),
                           add_account_false_ramdom(8),
                           add_account_false_ramdom(13),
                           add_account_false_ramdom(14)])
    ...
    @parameterized.expand([sub_account_false(10, 5),
                           sub_account_false(9, 4),
                           sub_account_false(14, 3),
                           sub_account_false(7, 2),
                           sub_account_false(6, 1),
                           sub_account_false(15, 0)])
    ...
    def test_add_account03(self, datas):
        ...
        with allure.step('点击账户,点击子账户,点击添加,输入错误邮箱格式+所有必填正确信息,点击确定'):
            ...
            self.click_link_text('子账户')
            self.click(AddAccount.account_add)
            self.type(AddAccount.account_email, datas[2])
        with allure.step(f'输入测试数据: 子账户不足6位字符的密码:{datas[3]}'):
            self.type(AddAccount.account_pwd, datas[3])
            self.type(AddAccount.account_phone, datas[4])
            self.type(AddAccount.account_firstname, datas[5])
            self.type(AddAccount.account_lastname, datas[6])
            self.click(AddAccount.account_language)
            self.hover_and_click(AddAccount.account_zh_cn, AddAccount.account_zh_cn)
            self.click(AddAccount.account_timezone)
            self.hover_and_click(AddAccount.account_timezone, AddAccount.account_time)
